...
import {incomeTopPct,rollCause} from "./roll.js";
...
export function decodeLife(s, shared) {
  ...
  const p = s.split(SEP);
  /* 11칸은 balding이 생기기 전에 뿌린 링크다. 그때 뽑힌 탈모 여부는 어디에도 안 남아서
     복원할 방법이 없다 — 숱 유지로 본다. 링크를 통째로 버리는 것보다는 낫다. */
  if (p.length !== 11 && p.length !== 12) return null;
