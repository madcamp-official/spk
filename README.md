# 🌏 환생 시뮬레이터 (Samsara Simulator)

실제 지구 인구 분포 확률 그대로 다음 생을 뽑는 웹 시뮬레이터.
리롤을 반복하다가 마음에 드는 생이 나오면 카드로 만들어 공유한다.

- **라이브**: https://life-reroll.com/ (구주소 life-reroll.madcamp-kaist.org 는 리다이렉트)
- **레포**: https://github.com/madcamp-official/spk (몰입캠프 26s-w3-c1-03)
- **스택**: 정적 HTML + CSS + ES 모듈. 웹에는 프레임워크·번들러가 없다 — 파일을 그대로 서빙한다.
  "모두의 환생 횟수"와 이벤트 수집은 Cloudflare Pages Functions + D1([functions/api/](functions/api/))이
  맡는다 — API 가 없으면 해당 타일만 숨겨진 채 나머지는 그대로 동작한다.
  (구 VM 시절의 [server/counter.js](server/counter.js) 가 이식 원본이다.)
- **구조**: pnpm workspace 모노레포. 국가 데이터·확률 로직은 [packages/core](packages/core)(TypeScript)에 있고
  웹·카운터 서버·Discord 봇([DiscordBot.md](DiscordBot.md))이 같은 소스를 쓴다. 빌드가 필요한 건 core뿐이다.

## 핵심 아이디어

환생 확률은 전부 현실 통계를 따른다. 인도로 태어날 확률 17.9%, 중국 17.5%,
대한민국 0.64%, 투발루는 약 400만분의 1. 등급 이름은 쓰지 않는다 — "SSR"보다
"0.0003% · 약 32만번 중 1번"이 더 세기 때문이다.

다만 **결과 화면에는 확률을 적지 않는다.** 리롤할 때마다 확률 줄이 나타났다 사라지며
바로 아래 "다시 환생하기" 버튼을 밀어내서, 연타하는 리듬이 끊겼다. 확률은 결과 화면에서
빼고 최고 희귀 기록·공유 카드·공유 문구에만 남겼다. 같은 이유로 리롤 연출(약 0.9초간
국기가 돌아가던 것)도 걷어냈다 — 버튼을 누르면 결과가 즉시 나온다.

## 기능 명세

| 기능 | 설명 |
|---|---|
| 환생 뽑기 | 198개국 인구 가중 추첨 + 성별(출생 성비)·도시/농촌(도시화율)·모국어·민족·종교·키·몸무게·IQ·사망 원인(사인)·탈모·기대수명·연 소득(USD). 누르면 즉시 결과가 나온다 |
| 희귀도 표현 | 등급 없음. 결과 화면에는 확률을 적지 않고(위 참조) 국가 인구에 따라 카드 테두리 색만 달라진다. 인구 500만 미만이면 컨페티. 확률(%)과 "약 N번 중 1번"은 최고 희귀 기록·공유물에만 남는다 |
| 오늘의 환생 운세 | 날짜+기기 시드 고정 롤. 하루 동안 같은 결과 + 운세 문장 (재방문 유도) |
| 환생 도감 | 태어나 본 나라 수집 현황 198칸 그리드 (수집욕 리텐션) |
| 공유 | 결과 카드 PNG(1080x1350) 저장, 텍스트 복사, 모바일 공유 시트(이미지 첨부) |
| 체류 시간 계측 | 결과를 몇 초 들여다보다 리롤했는지 자동 수집. UI 없음 (이모지 평가를 대체) |
| 한 줄 제안 | 하단 "이런 항목도 넣어 주세요" 입력창 — 유일한 직접 의견 통로 |
| 누적 통계 | 나의 환생 횟수, 도감 진행률, 최고 희귀 기록 (localStorage) |
| 모두의 환생 횟수 | 방문자 전체가 뽑은 환생의 합계. `/api/counter`로 읽고 환생마다 1씩 올린다 |
| 국기 표시 | 윈도우에는 국기 이모지 글리프가 없어 "KR" 두 글자로 나온다. 국기 코드포인트만 담은 Twemoji 웹폰트를 얹어 전 OS에서 국기가 보인다 |

## 파일 구조

한 파일에 1,469줄이 쌓여 기능별로 쪼갰고, 모듈이 20개를 넘어가며 역할별 폴더로 묶었다.
그리고 Discord 봇([DiscordBot.md](DiscordBot.md))이 국가 데이터·확률 로직을 웹과 공유해야 해서
**pnpm workspace 모노레포**가 됐다. 웹은 여전히 빌드가 없다 — 브라우저의 ES 모듈을 그대로 쓴다.
빌드가 붙은 것은 `packages/core`(TypeScript) 하나뿐이다.

```
pnpm-workspace.yaml   워크스페이스 선언 (apps/*, packages/*)
package.json          루트 스크립트 (dev · build:core)  ⚠ type:module 넣지 말 것 (server가 CJS)

packages/core/        국가 데이터 · 생 샘플링 · 확률 — 플랫폼 무관 순수 로직
  src/*.ts              TypeScript 소스 (정본)
    types.ts    Country · Life · Trait  (DiscordBot.md §D·§G)
    config.ts   밸런스 수치 전부 (§A.8) — 여기 밖에 하드코딩 금지
    util.ts     난수원(RNG 싱글턴) · 분포 · clamp · isoCode
    data.ts     198개국 데이터 (RAW·REL·BODY)
    roll.ts     환생 뽑기 — 국가·성별·민족·체격·IQ·소득
  dist/*.js             빌드 산출물. **커밋한다** (VM은 node 없이 배포하므로)
  package.json          dependencies 비어 있음 ← "core는 어디에도 의존하지 않는다"의 증명

apps/web/             정적 웹 (프레임워크·번들러 없음)
  index.html          마크업만. <script type="module">로 app/main.js
  css/style.css       전체 스타일
  TwemojiCountryFlags.woff2   국기 웹폰트
  core/               ← packages/core/dist 복사본 (빌드가 생성, .gitignore)
  app/
    main.js     조립 + 리롤 버튼 + 전역 단축키 (진입점)
    core/       웹 토대
      util.js     DOM 단축 · 표시 포맷 + core의 난수원을 재수출
      state.js    localStorage 상태(ST) + 지금 보고 있는 생(session)
    i18n/       표시 언어·번역 사전 (아래 "언어 규칙" 참고)
      i18n.js       표시 언어(ko/en/ja/zh/es/pt)·UI 문구 사전·전환 위젯
      i18n-terms.js core 데이터의 언어·종교·민족 값 번역 사전
    engine/     웹 전용 생 로직
      permalink.js  생 ↔ 공유 URL 인코딩·디코딩
      lifepool.js   서버가 서명해 둔 생 버퍼 (없으면 로컬 뽑기로 폴백)
      titles.js     칭호·마일스톤 (ST 의존)
    ui/         화면·상호작용 (render · share · dex · fortune · odds · suggest · effects · flags · counter · ads · titlechip · achievements)
    analytics/  행동 계측 (track · reach)
  og-image.png        링크 미리보기 배너 — og-image-<lang>.png 로 en/ja/zh/es/pt 도 있다
  en.html … pt.html   언어별 공유 랜딩. tools/make_share.py 가 생성

MT/                   /MT 팀 리롤 (자립 페이지 — core·i18n·서버 API를 쓰지 않는다)
  index.html          마크업만. <script type="module">로 app/main.js
  css/mt.css          apps/web/css/style.css 에서 이 페이지가 쓰는 것만 옮겨 온 사본
  people/teams.js     ★ 팀 데이터 (25팀 · 15명 · 주차 · 아이콘 · 금주의 픽 · id) — 고칠 곳은 여기 하나
  app/                util · state · roll · render · tally · effects · main
  icon/               팀 아이콘 PNG 21개. 파일명 = teams.js 의 icon 값 (한글 파일명 있음)
  bgm/                팀 전용 BGM. 파일명 = teams.js 의 bgm 값 (지금은 히든 팀 하나)
  img/                (비어 있음) 나중에 인물 사진을 붙일 자리

apps/bot/             Discord 봇 (독립 프로세스). 기준 문서는 DiscordBot.md
  src/commands/       /환생 · /여권 · /덱 · /명명 · /도감 · /배틀
  src/db/             pg + 마이그레이션 (§G 스키마)
  src/lib/            인생 요약(템플릿+LLM) · 임베드 · 표시 문구
  src/verify.ts       WASM Postgres로 도는 자체 검증 (비밀값 불필요)

functions/          Cloudflare Pages Functions — /api/* (D1 사용). server/counter.js 의 후계
server/counter.js   카운터·이벤트 수집 (구 VM 전용 — Pages 이전 후 참조용으로만 남김)
server/d1/          D1 스키마·시드 (schema.sql · seed.sql)
tools/              dev-server.mjs · sync-core.mjs · make-d1-seed.mjs · make_og.py · make_share.py · analyze.py
wrangler.toml       Pages 설정 (출력 디렉터리 · D1 바인딩)
deploy.sh           (구 VM 배포 — Pages 전환 후 안 씀)
deploy-bot.sh       Discord 봇 배포 (systemd 상주 — HTTP Interactions 전환 예정)
```

### core를 브라우저가 어떻게 찾는가 (중요)

웹에는 번들러가 없어서 브라우저가 import를 **URL**로 푼다. pnpm의 `node_modules` 심볼릭
링크는 브라우저에 보이지 않으므로, core는 **웹 트리 안의 경로**로 존재해야 한다.

```
빌드:  packages/core/dist  →  apps/web/core        (tools/sync-core.mjs)
배포:  packages/core/dist  →  /var/www/life-reroll/core   (deploy.sh)
웹:    app/ui/render.js 의 `../../core/roll.js`  →  /core/roll.js
```

- 상대 경로(`../../core/`)를 쓰는 이유(유래): 실험판(lab)을 `/lab/` 하위에 얹을 때 절대
  경로 `/core/`면 프로덕션 core 를 물어 격리가 깨졌다. lab 은 없어졌지만 상대 경로는
  하위 경로 어디에 얹어도 동작하므로 그대로 둔다.
- `apps/web/core`는 산출물이라 `.gitignore`에 있다. 커밋되는 정본은 `packages/core/dist`다.
- **core 소스를 고치면 `npm run build:core`를 반드시 다시 돌린다.** 안 그러면 웹은 옛 core를 본다.

의존 방향은 한쪽으로만 흐른다: `packages/core`는 아무것도 import하지 않고(웹·Discord 모두 모름),
웹은 `core → i18n/engine → ui/analytics → main` 순으로 쌓인다. **순환 참조가 생기면 되돌릴 것.**

## 로컬 실행

```bash
pnpm install          # 최초 1회
pnpm dev              # core 빌드 + 정적 서버 → http://127.0.0.1:8791
```

`/api/*`는 붙지 않으므로 "모두의 환생 횟수" 타일은 숨겨지고 생은 로컬에서 뽑힌다(서명 없음).
예전에 `python -m http.server`로 띄우던 것과 같은 상태다. 카운터까지 보려면 별도로
`node server/counter.js`를 띄우고 nginx처럼 `/api/`를 프록시해야 한다.

`/MT`(팀 리롤)는 `apps/web` 밖에 살지만 같은 서버에 얹혀 나온다 →
<http://127.0.0.1:8791/MT/>. `tools/dev-server.mjs`의 `MOUNTS`가 프로덕션 nginx와
같은 URL이 되게 맞춰 준다. API도 core도 안 쓰므로 이쪽은 빌드 없이 바로 돈다.

## /MT — 팀 리롤 (배포 중단, 코드는 유지)

> **지금은 배포하지 않는다.** `deploy.sh` 의 MT 절을 걷어냈다. 레포의 `MT/` 는 코드·아이콘·
> 음원까지 그대로 있고 로컬(`npm run serve` → `/MT/`)에서는 계속 돈다. 다시 올리려면
> 그 커밋을 되돌리는 편이 안전하다 — 자산 목록·확장자 화이트리스트·검증 블록이 한 덩어리다.
> 이미 서버에 올라간 배포본은 스크립트가 지우지 않으므로 `rm -rf /var/www/life-reroll/MT` 가 필요하다.


몰입캠프 4주간의 **25개 팀 중 하나**를 뽑는다. 환생 시뮬레이터의 화면을 그대로 따라가되
나라 자리에 팀 이름이, 칩 자리에 그 팀이었던 사람들이 들어간다. 뽑기 · 결과 · 누적 횟수까지만
있고 공유·업적·운세·i18n·광고는 없다(공유 시트와 결과 카드는 만들었다가 걷어냈다).
`?t=<팀id>` 로 특정 팀을 보여주는 길만 남아 있다 — 링크를 만들어 주는 버튼은 없다.

- **팀을 고치려면** [MT/people/teams.js](MT/people/teams.js) 하나만 본다. 명단(`PEOPLE`)은
  팀에서 파생되므로 따로 적을 곳이 없다.
- **`id`는 바꾸지 않는다.** 링크가 `?t=<id>`라서, 바꾸는 순간 이미 건넨 주소가 전부
  "없는 팀"이 된다. 이름·아이콘은 언제 고쳐도 링크가 안 죽는다.
- **아이콘**은 `MT/icon/*.png`, 팀의 `icon` 필드가 파일명이다. 없는 팀(5팀)은 `emoji` 로
  그린다. `emoji` 는 아이콘이 있는 팀에도 남겨 둔다 — 파일이 없을 때의 폴백이다.
  파일명에 한글이 섞여 있어 URL은 `iconURL()`이 `encodeURIComponent` 로 만든다.
- **주차 라벨**(`ROUNDS`)은 이 레포가 `26s-w3-c1-03`이고 3번째 묶음에 환생 시뮬레이터가
  있는 것에서 역산했다. 다르면 그 한 줄만 고치면 된다.
- 희귀도 축은 **금주의 픽**(`pick: true`) 하나뿐이다 — 8팀이 지정돼 있고, 뜨면 테두리가
  금색이 되고 컨페티가 터진다. **확률은 안 건드린다**: 25팀 전부 4%이고 픽은 "떴을 때
  특별한 것"이지 "덜 나오는 것"이 아니다. 주마다 바꾸려면 `teams.js`의 `pick` 표시만 옮긴다.
- 기록은 **누적 횟수**다(`mt_state.counts = {팀id: 횟수}`). 사람 횟수는 그 사람이 속한 팀들의
  횟수 합으로 파생시킨다 — 따로 저장하지 않아야 팀 데이터를 고쳐도 어긋나지 않는다.
  한 번 뽑으면 1~4명이 함께 올라가므로 사람 횟수의 합은 리롤 횟수보다 크다.
- **히든 팀**(`secret: true`, 현재 Synsory 하나): 확률은 다른 팀과 같은 1/26이지만
  한 번이라도 뽑기 전까지 집계·통계·확률 문구에서 통째로 빠진다(그 팀에만 있는 사람도 함께).
  감춤 판정은 `state.js` 의 `revealed()` **한 곳**이고, 화면에 "전체 몇 팀"을 적는 자리는
  전부 `visibleTeams()` 를 거쳐야 한다 — `TEAMS.length` 를 그대로 쓰면 숫자로 존재가 샌다.
  확률 문구도 감출 팀이 남아 있는 동안은 숫자를 빼고 "모든 팀이 똑같아요"로만 적는다
  (25팀·4%로 적으면 거짓, 26팀·3.8%로 적으면 노출이라 둘 다 피한다).
- **팀 BGM**(`bgm:` 필드): 그 팀이 뜨면 `MT/bgm/` 의 mp3 를 틀고, 다른 팀으로 넘어가면 끈다.
  `Audio` 객체는 처음 필요할 때 만들어서, 히든 팀을 못 만난 사람은 mp3 를 아예 안 받는다.
  `play()` 는 사용자 조작 없이 막히므로(`?t=` 링크로 바로 들어온 경우) 실패를 삼킨다 —
  화면은 이미 다 그려져 있어 잃는 것은 소리뿐이다.
  ⚠ 새 확장자를 `MT/` 에 넣을 때는 **`deploy.sh` 의 MT 자산 목록과 `tools/dev-server.mjs`
  의 MIME 표**를 같이 늘려야 한다. 둘 다 확장자 화이트리스트라, 안 늘리면 로컬에선 되는데
  배포하면 파일이 없다(현재 `.mp3`·`.m4a`).
- **소리**: 리롤 버튼 클릭음(효과음 채널) · 팀 전용 BGM · "또 나왔다"(BGM 채널). 우선순위는
  팀 전용 > 또 나왔다 > 무음이고, 겹치면 둘 다 안 들려서 BGM 채널은 항상 한 곡만 흐른다.
  재등장 판정은 `recordRoll()` **전에** 해야 한다 — 센 뒤엔 방금 것까지 포함돼 늘 참이 된다.
- **기록 일괄 초기화**(운영자): 서버에 저장하는 것이 없어서 방법은 `state.js` 의 `EPOCH` 를
  올리는 것 하나뿐이다. 세대가 다르면 다음 방문 때 저장분을 통째로 버린다. **히든 팀 발견
  기록도 같이 사라지므로**(같은 곳에 산다) 모두가 Synsory 를 다시 처음부터 찾게 된다.
- **기록 초기화**(방문자): 집계 카드 안의 `기록 초기화` 버튼. `mt_state` 키 하나만 지운다 —
  브라우저 설정의 "사이트 데이터 삭제"와 달리 환생 시뮬레이터(`rebirth_state`)를 건드리지
  않는다(같은 출처라 그쪽 삭제는 도감·업적까지 날린다). 지운 뒤에는 다시 그리지 않고
  `location.reload()` 한다 — 첫 화면 문구가 index.html 에만 있어서 손으로 복원하면 언젠가 어긋난다.
- 코드를 `apps/web`과 공유하지 않는다. 그쪽을 물면 core·i18n·서명이 통째로 딸려 오는데
  이 페이지엔 쓸 데가 없다. 대신 `css/mt.css`는 `style.css`의 사본이라 **원본 디자인을
  바꾸면 두 곳을 봐야 한다**(값은 그대로 옮겨 두었다).

배포는 `deploy.sh`가 함께 처리한다(`2-1. /MT 팀 리롤` 절). 대문자 `MT` 그대로여야 한다 —
리눅스 파일시스템은 대소문자를 구분해서 `/mt`로는 안 열린다.

## 언어(i18n) 규칙

우상단 전환 위젯으로 표시 언어를 고른다. 현재 지원: **한국어(원문) · English · 日本語 · 中文(간체) · Español · Português**.
한국어 문장이 원문이자 사전의 키다 — [apps/web/app/i18n/i18n.js](apps/web/app/i18n/i18n.js)의 `STR`(UI 문구)과
[apps/web/app/i18n/i18n-terms.js](apps/web/app/i18n/i18n-terms.js)의 `TERMS`(data.js의 언어·종교·민족 값)에
`[영어, 일본어, 중국어, 스페인어, 포르투갈어]` 순서로 번역이 붙는다. 나라 이름만 사전이 없다 — 국기 이모지에서 ISO
코드를 뽑아 `Intl.DisplayNames`가 만든다. 선택은 `localStorage`(`rebirth_lang`)에 남고,
전환은 저장 후 새로고침이다.

**첫 방문 언어는 접속 국가로 정한다** (`pickLang`). 순서는 ① 저장된 선택(항상 최우선) →
② 접속 국가 → ③ **기본 영어**. 국가 매핑은 `KR→ko`, `JP→ja`, `CN·TW·HK·SG→zh`, 스페인어권(ES·MX·AR 등)→`es`, 포르투갈어권(BR·PT·AO 등)→`pt`, 나머지 전부 `en`이다.
국가는 nginx가 Cloudflare의 `CF-IPCountry`를 `geo` 쿠키로 내려주고(HttpOnly 아님),
i18n.js가 첫 렌더 전에 **동기적으로** 읽는다 — 서버 왕복도, 언어 깜빡임도 없다
(Cloudflare가 HTML을 캐시하지 않아 `cf-cache-status: DYNAMIC` 방문자마다 자기 국가가 담긴다).
한 번 언어를 직접 고르면 그 선택이 저장되어 국가 판정을 덮는다. `CF-IPCountry`는
`/api/`로도 넘어가므로(카운터 서버가 읽을 수 있음), 나중에 국가별 지표에도 쓸 수 있다.

> ⚠️ **규칙: 화면에 보이는 문구·데이터 값을 추가하거나 바꾸면, 지원되는 모든 언어의
> 번역을 같은 커밋에서 함께 추가·수정해야 한다.**
>
> - 새 UI 문장 → `i18n.js`의 `STR`에 (숫자 등 가변부는 `{x}` 자리표시자로)
> - `data.js`에 새 나라·언어·종교·민족 값 → `i18n-terms.js`의 `TERMS`에
> - 새 언어를 추가할 때는 `LANGS`·`STR`·`TERMS`·`CONT`·`FOOTER`·`index.html`의
>   전환 버튼을 한 번에 채운다 — 기존 키 전부에 새 언어 칸이 생겨야 한다
>
> 사전에서 빠진 문구는 조용히 죽지 않고 **한국어 그대로 노출된다**(누락을 바로
> 발견하라는 의도). EN/JA 화면에 한국어가 보이면 이 규칙이 깨진 것이다.

> **번역 만드는 법(es·pt 추가 때 쓴 방식):** 언어 이름(TERMS의 언어 섹션)은 `Intl.DisplayNames`로
> 자동 생성했다(BCP-47 코드 맵 → es/pt). 민족은 영어 칸이 이미 로마자 고유명사(Yoruba 등)라
> es/pt에서도 같으므로 영어값을 복사하고, 서술형(백인·흑인·기타·메스티소 등)만 손번역했다.
> UI 문구(STR)·종교는 손번역. **IX가 언어 수와 정확히 맞아야 한다** — `LANGS`에 언어를 더하면
> `IX` 계산과 STR/TERMS의 모든 칸을 함께 늘려야 하고, 어긋나면 새 언어가 옆 칸(예: 일본어)을
> 읽어 조용히 틀린다.

> ⚠️ **`index.html`을 파일로 직접 열면(`file://`) 더 이상 동작하지 않는다.** ES 모듈은
> CORS 때문에 http로 서빙해야 한다. 아래 "로컬 실행"의 정적 서버를 쓸 것.

`window.__app`에 주요 내부(DATA, rollLife, renderLife 등)를 열어 두었다 —
콘솔에서 만져 보거나 자동 검증에서 쓴다.

## 반응 수집 설계 (배포 → 반응 → 개선 루프)

### 유입 추적 규약

홍보 채널마다 URL에 `?ref=` 파라미터를 붙여 배포한다. **표준 태그는 아래 하나로 고정한다** —
같은 채널을 `kakao`/`Kakao`처럼 다르게 쓰면 [analyze.py](tools/analyze.py)에서 두 줄로 갈려
합계를 손으로 더해야 한다. 값 규칙은 소문자·`[a-z0-9_-]` 1~24자 ([apps/web/app/analytics/track.js](apps/web/app/analytics/track.js)의 `REF` 검증).

| 채널 | 링크 |
|---|---|
| 카카오톡 | `https://life-reroll.com/?ref=kakao` |
| X | `https://life-reroll.com/?ref=x` |
| 인스타그램 | `https://life-reroll.com/?ref=instagram` |
| 디스코드 | `https://life-reroll.com/?ref=discord` |
| 레딧 | `https://life-reroll.com/?ref=reddit` |
| 에브리타임 | `https://life-reroll.com/?ref=everytime` |

> ⚠️ **`ref`(유입)와 `via`(공유 채널)는 다른 필드다.** `ref=kakao`는 *내가* 카톡에 뿌린 홍보
> 링크를 누른 사람(첫 유입, ACQUISITION 표)이고, `via=kakao`는 *사용자가* 앱의 "카톡 공유"
> 버튼으로 퍼뜨린 링크를 누른 사람(2차 확산, REFERRAL 표)이다. 이름은 같아도 서로 다른 표에서
> 읽히고, 유입 링크에는 `via`가 없어 겹쳐 세지 않는다.
>
> 인스타는 피드 캡션의 링크가 클릭되지 않는다 — `?ref=instagram` 유입은 프로필 링크·스토리
> 링크 스티커에서만 나온다. 둘을 가르려면 `?ref=ig_bio`·`?ref=ig_story`로 나눈다.

사용자가 앱에서 공유하면 자동으로 `?ref=share&v=a|b`가 붙는다.
`v`는 공유 문구 A/B 테스트 변형이다 (기기별 고정 배정).

- **a (스토리형)**: "나는 브라질 도시에서 여자로 태어났다"
- **b (성과형)**: "확률 0.26%의 환생 뽑기 성공! 대한민국"

**한 기기는 유입자이면서 동시에 공유자다.** 그래서 두 값을 분리해서 싣는다.

| 필드 | 뜻 | 쓰는 곳 |
|---|---|---|
| `vin` | 나를 **데려온** 공유 문구 (URL `?v=`, 첫 유입값 고정) | 문구 A/B 비교 |
| `v` | 내가 **공유할 때 쓸** 문구 (기기별 무작위 배정) | 공유 문구 선택 |

둘을 한 필드로 합치면 유입 분포가 자기 동전던지기에 묻혀 항상 50:50이 된다.
문구 성과는 반드시 `vin`으로 읽는다.

### 수집 이벤트 (AARRR 매핑)

모든 이벤트에 `ref`·`vin`·`v`가 자동으로 붙는다.

| 단계 | 이벤트 | 속성 | 읽는 법 |
|---|---|---|---|
| **Acquisition** · 획득 | `visit` | ref, vin | 채널별 신규 방문 수 |
| **Activation** · 활성화 | `activate` | ms, returning | 첫 방문에서 첫 리롤까지 걸린 시간. 이 이벤트가 없는 `visit`은 버튼도 안 눌러 본 이탈 |
| **Retention** · 리텐션 | `fortune`, `collection_open` | first / owned | `first=true` 비율 = 매일 첫 방문 |
| **Referral** · 추천 | `share_text`, `share_card`, `share_native` | country, prob | 발생 수 대비 `ref=share` 유입 수 = 바이럴 계수 |
| **Revenue** · 매출 | — | — | **없음. 무료 서비스라 이 단계는 비어 있다** (아래 참고) |

수집 통로(퍼널 단계는 아니지만 개선 근거가 되는 이벤트):

| 이벤트 | 속성 | 무엇을 알려 주나 |
|---|---|---|
| `share_open` | country, prob | 공유 시트를 **열었다**. 아직 공유가 아니다 — 바이럴 계수 분모에 넣지 말 것 |
| `dwell` | ms, country, prob, shared, reason, roll_idx | **이 생이 마음에 들었나.** 오래 볼수록 좋았던 것 |
| `suggest` | text, country | 사용자가 직접 요청한 항목 |
| `exit` | rolls, activated | 떠나기 전 몇 번 굴렸나 (평균 리롤 수) |
| `roll` | country, prob, idx, since_prev_ms, quick | 결과 분포 + 세션 안 리롤 리듬. `prob`으로 "희귀한 생일수록 더 공유되는가"를 본다 |
| `reach` | section, rolls_so_far | 화면을 어디까지 내려 봤나(`odds`/`suggest`/`footer`). 히어로에서 멈추는 사람과 끝까지 보는 사람을 가른다 |

#### 이모지 평가(😐/😂/🔥)를 걷어내고 체류 시간으로 바꾼 이유

이모지 평가는 **말한 의견**이고, 체류 시간은 **드러난 의견**이다. 이 표의 첫 줄이
"행동 데이터가 곧 의견"인 이유가 그것이다. 이모지는 클릭을 요구해서
① 응답률이 낮고(누른 사람만 집계) ② 이미 마음에 든 사람 쪽으로 표본이 쏠린다.
반면 체류 시간은 **모든 생에서 100% 수집되고, 사용자에게 아무것도 요구하지 않는다.**

```
dwell {ms:3540, country:"인도네시아", shared:false}  → 3.5초 들여다봄
dwell {ms:1603, country:"수단",      shared:true }  → 보고 공유까지 함 ★
dwell {ms:537,  country:"중국",      shared:false}  → 0.5초 만에 리롤 = 심심함
```

읽는 법: **희귀도(prob) 구간별 `ms` 중앙값**을 본다. 희귀한 생인데도 체류가 짧으면
연출이 약하다는 뜻이고("너무 빨리 떠나면 결과 화면이 심심하다는 뜻"),
`shared:true`인 생의 특징을 모으면 "공유가 터지는 결과 유형"이 그대로 나온다.

`reason`은 그 생을 왜 떠났는지다: `reroll`(다시 굴림)·`fortune`(운세)·`exit`(이탈)·`idle`(자리 비움).
`reroll`·`fortune`은 사용자가 **돌아와서** 뭔가 한 dwell이라 진짜 몰입을 반영하는 신뢰 구간이고,
`exit`·`idle`은 떠나거나 자리를 비운 쪽이라 신뢰가 낮다 — 희귀도 비교는 앞의 둘로만 한다
([tools/analyze.py](tools/analyze.py)가 그렇게 나눠 출력한다). `idle`은 **굴려 놓고 딴 일로 넘어간**
경우다: 탭이 앞에 떠 있어 `visibilitychange`가 안 떠도, 마지막 상호작용 뒤 20초가 지나면
그때 dwell을 닫고 그 20초까지만 인정한다 — 안 자르면 "심심해서 떠난 시간"이 "오래 들여다본 것"으로
뒤집혀 잡힌다. `roll_idx`는 이 생이 세션 안 몇 번째 리롤인지라(남의 생은 0), 긴 dwell이
초반의 신기함인지 후반의 방치인지 가른다.

#### Revenue 단계에 대해

수업 자료의 AARRR은 5단계지만 이 서비스에는 매출이 없다. 억지로 채우는 대신
비워 두고, "얼마나 멀리 퍼지는가"를 묻는 주제 2에서는 **Referral을 최종 지표로**
본다. 굳이 붙인다면 후원 링크나 결과 카드 굿즈 정도가 후보이며, 현재 계획에는 없다.

#### Activation을 `roll`로 재지 않는 이유

첫 리롤은 사실상 모든 방문자가 누른다. 그래서 `roll`을 Activation으로 잡으면
Acquisition과 거의 같은 숫자가 나와 퍼널이 아무것도 걸러내지 못한다. 대신
**`activate`의 `ms`(첫 리롤까지 걸린 시간)** 로 잡는다. 이 값이 길면 첫 화면 설명이
길다는 뜻이고, `visit`은 있는데 `activate`가 없으면 버튼조차 누르지 않고 떠난 것이다.

### 공유 링크 (짧은 코드)

공유 링크는 `?s=Xa9k2p` 7자 코드다. 예전엔 생 값을 URL에 통째로 실었지만
(`?l=KR-1-0-…&sig=…`) 링크만 봐도 생이 읽혀 지저분했다. 이제 서버가 생을 저장하고
코드만 발급한다:

```
공유 시 :  POST /api/share {l, sig}  → {code}   (서명 통과한 생만 저장)
링크 열 때: GET  /api/shared?s=code   → {l, sig}  (코드로 생을 꺼냄)
```

- 저장은 `shares.jsonl`(append-only, 부팅 때 Map으로 로드). **런타임 데이터라 배포에 안 실린다** —
  링크를 영구 보존하려면 이 파일을 백업해야 한다. 재시작해도 파일에서 다시 읽어 링크가 살아남는다.
- **서명(`sig`)을 통과한 생만 저장**하므로 남이 아무 값이나 넣어 채울 수 없다. 그래서 코드로
  꺼낸 생은 다시 검증하지 않는다 — 저장됐다는 것 자체가 진짜라는 증거.
- 옛 `?l=&sig=` 링크도 계속 열린다(하위호환). `LIFE_SECRET`이 꺼진 로컬에서는 서버가 코드를
  못 주므로 생 없는 링크로 나간다(받는 쪽이 새로 뽑는다).

### 수집 백엔드 (자체 수집)

외부 스크립트 0개. 이벤트는 같은 도메인의 `POST /api/track`으로 가서
[server/counter.js](server/counter.js)가 JSONL 한 줄로 append한다.

```
브라우저          track() → 메모리 큐 → (3초 유휴 | pagehide) → sendBeacon
nginx             /api/ → 127.0.0.1:1558
counter.js        POST /api/track → events.jsonl 에 한 줄씩 append
```

**지연 0 규약** (리롤 체감을 지키는 유일한 이유):

- 클릭 경로에서 하는 일은 **큐에 push하는 것뿐**이다. 전송은 유휴/이탈 시점에 몰아서 한다
- `await` 없음. `navigator.sendBeacon`만 쓰고 응답을 보지 않는다. 전송 실패는 무시한다
- `exit`·마지막 `dwell`은 `pagehide` 때만 존재하므로 그 시점에 반드시 `flushEvents()`
- 큐는 500개에서 오래된 것부터 버린다 (전송이 계속 실패해도 메모리가 안 샌다)

**`ref`/`vin`/`v` 각인은 `track()` 안에서 끝난다.** 큐에는 각인된 뒤의 이벤트가 들어간다.
큐 어댑터를 손볼 때 이 순서를 깨면 `vin`이 사라지고, 그러면 문구 A/B를 영영 못 읽는다
(조용히 실패하므로 눈치채기 어렵다).

**개인정보**: 원 IP는 저장하지 않는다. `CF-Connecting-IP`를 **매일 바뀌는 랜덤 솔트**로
해시해 `ip_h`로만 남긴다 → "오늘의 고유 방문자"는 세지만 어제와는 이어붙일 수 없다.

**레이트리밋**: `/api/track`은 배치라 counter와 트래픽 모양이 다르다(한 번에 최대 50개,
pagehide 때 몰림). nginx의 counter용 `limit_req` 존을 그대로 쓰면 정상 배치가 503으로
잘리므로, counter.js가 **IP 해시당 분당 240개**로 자체 제한한다
(`TRACK_RATE_PER_MIN`). 한 IP의 도배가 다른 사용자를 막지 않는다.

배포 후 확인:

```bash
tail -f /var/lib/life-reroll/events.jsonl    # 리롤 2~3회 → visit/roll/dwell이 쌓이는지
```

### 지표 보기

[tools/analyze.py](tools/analyze.py)가 `events.jsonl`을 읽어 AARRR 퍼널을 한 번에 뽑는다.
의존성 없음(표준 라이브러리만) — VM에 jq나 pip 없이도 돈다.

```bash
# VM에서 바로
python3 tools/analyze.py                      # /var/lib/life-reroll/events.jsonl 자동 탐색
python3 tools/analyze.py --since 2026-07-18   # 그 날짜부터만
python3 tools/analyze.py --suggests           # 사용자 제안 전문까지

# 로컬에서 보려면 파일만 내려받아서
scp camp-15:/var/lib/life-reroll/events.jsonl .
python3 tools/analyze.py events.jsonl
```

출력에 들어가는 것: 채널별 유입(`ref`) · 활성화율과 첫 리롤까지 걸린 시간 ·
세션당 리롤 수 · 바이럴 계수 · 문구 A/B(`vin`) · 희귀도 구간별 체류 시간 ·
공유가 터진 나라 · 제안 전문.

읽을 때 주의할 것을 도구가 직접 말해 준다:

- 표본이 30건 미만이면 **A/B 승자를 정하지 말라고** 경고한다
- 희귀도별 체류 시간에는 **연출 교란 경고**가 붙는다 (아래 참조)
- `ip_h`가 1개로 뭉쳐 있으면 **nginx가 `CF-Connecting-IP`를 안 넘긴다는 신호**라고 알려 준다
- 배포 점검용 합성 이벤트(`probe`·`test` 등)는 기본으로 빼고 센다 (`--keep-test`로 포함)
- **`share_open`은 공유가 아니다.** 시트를 열고 닫은 사람까지 공유자로 세면 바이럴 계수가
  실제보다 낮게 나온다. "열기 N건 → 실제 공유 M건 (완료율)"로 나눠서 보여준다
- **하루의 경계는 KST 자정**이다. `server/counter.js`의 솔트 회전과 이 도구의 날짜 버킷은
  **같은 기준을 써야 한다** — 어긋나면 한 버킷에 서로 다른 솔트의 해시가 섞여 고유 방문자가
  부풀려진다. 둘 중 하나만 바꾸지 말 것
- **카운터를 재시작하면 그날 고유 방문자가 갈린다.** 솔트가 메모리에만 있어서다
  (IP를 남기지 않으려는 의도적 선택)

**자동화 트래픽은 애초에 수집되지 않는다.** 헤드리스 브라우저(`navigator.webdriver`)는 항상
버튼을 누르고 수십 번씩 굴려서 활성화율·세션당 리롤·공유수를 통째로 왜곡한다. 그래서 이벤트
전송과 "모두의 환생 횟수" 증가만 건너뛴다 ([app/core/util.js](app/core/util.js)의 `isAutomated`).
화면 동작과 스니펫 발화는 그대로라 자동화 테스트는 여전히 이벤트를 확인할 수 있다.

### 외부 도구를 쓸 경우 (선택)

`index.html` 맨 아래 주석 위치에 GA4 / PostHog / Plausible 스니펫을 붙이면 `track()`이
전역 객체를 감지해 **자체 수집과 병행**해서 보낸다. 스니펫이 없으면 전부 no-op이라
앱은 그대로 동작한다. 단, 붙일 거면 반드시 `defer` + 첫 페인트 이후 로드 —
외부 스크립트를 크리티컬 패스에 넣지 않는다.

#### GA4 (붙어 있음, G-168CKEDWJR)

GA4로 가는 복사본은 [apps/web/app/analytics/track.js](apps/web/app/analytics/track.js)의 `toGA4()`가 이름을 고쳐 보낸다.
**events.jsonl·analyze.py의 내부 이름은 그대로다** — 여기를 바꿀 때 둘이 어긋나면
안 되는 게 아니라, 애초에 서로 독립이다:

| 내부 이벤트 | GA4에서 |
|---|---|
| `visit` | 안 보냄 (자동 `page_view`와 중복) |
| `share_text`/`share_kakao`/`share_insta`/`share_x`/`share_native`/`share_card` | `share` + `method`(clip/kakao/insta/x/native/card) |
| `share_open` | 그대로 (공유가 아니라 시트 열기 — 합치면 완료율을 못 본다) |
| `dwell`의 `ms` | `dwell_ms` |
| `activate`의 `ms` | `ms_to_first_roll` |
| `exit`의 `rolls` | `session_rolls` |

**콘솔 설정 없이는 파라미터가 안 보인다.** 이벤트 수만 나오고 `country`·`method` 등은
버려진다. GA4 관리 → 맞춤 정의에서 등록해야 한다 (반영은 등록 *이후* 데이터부터):

- 맞춤 측정기준(이벤트 범위): `ref` `v` `vin` `via` `country` `method` `reason`
  `returning` `first` `shared` `fromLink` `activated` `quick` `section`
- 맞춤 측정항목: `prob`(%) `dwell_ms`(ms) `ms_to_first_roll`(ms) `session_rolls` `owned`
  `idx`(리롤 번호) `since_prev_ms`(리롤 간격) `roll_idx`(dwell의 리롤 번호) `rolls_so_far`
- 키 이벤트 지정: `activate`(진짜 Activation 문턱), `share`(Referral의 분자)

`ref`·`vin`·`v`·`via`는 이벤트 파라미터일 뿐 아니라 track.js 로드 시 **user_property로도** 심는다
([apps/web/app/analytics/track.js](apps/web/app/analytics/track.js)의 `gtag("set","user_properties",…)`). 이벤트 범위
값은 activate 이후 이벤트에만 붙어 "버튼도 안 누르고 떠난 방문"의 채널이 GA에서 비지만,
user_property는 자동 `page_view`까지 세션 전체에 붙어 그 이탈까지 채널로 잡힌다. 그래서 이 넷은
**맞춤 정의에 '사용자 범위'로도 한 번 더 등록**한다 (위 이벤트 범위 등록과 별개 슬롯이다). 배포
링크에 utm을 붙이지 않고도 GA가 유입 채널을 세션 단위로 가르게 하는 통로다.

## 빌드 · 배포 명령

```bash
# 로컬 실행 (위 "로컬 실행" 참고). core를 빌드한 뒤 apps/web 을 서빙한다.
pnpm dev                            # → http://127.0.0.1:8791
pnpm run build:core                 # core만 다시 빌드 (dist + apps/web/core 동기화)
pnpm run check                      # core 타입 검사만 (산출물 없음)

# 카운터까지 같이 띄우려면 (선택)
COUNTER_FILE=./counter.json node server/counter.js   # → 127.0.0.1:1558

# OG 썸네일 재생성 (수정 시). 6개 언어(ko/en/ja/zh/es/pt)를 한 번에 만든다.
sudo apt install python3-pil fonts-noto-cjk   # CJK 글리프가 없으면 일본어·중국어가 두부(□)로 나온다
python3 tools/make_og.py            # 전부 → og-image.png, og-image-en.png …
python3 tools/make_og.py en ja      # 일부만

# 언어별 공유 랜딩 재생성 (og 문구를 고쳤을 때)
python3 tools/make_share.py         # → en.html … pt.html
```

### 언어별 공유 링크 (해외 커뮤니티 배포용)

크롤러(Reddit·트위터·디스코드)는 **정적 HTML의 `<meta og:*>`만 읽고 JS는 실행하지 않는다.**
`index.html`의 og는 한국어라 그대로 뿌리면 미리보기가 한국어로 뜬다. 그래서 언어별 랜딩을 둔다:

| 뿌릴 주소 | 크롤러가 보는 것 | 사람이 가는 곳 |
|---|---|---|
| `life-reroll.com/en` | 영어 배너·제목·설명 | JS가 언어를 저장하고 `/` 로 이동 → **영어 앱** |
| `/ja` `/zh` `/es` `/pt` | 각 언어 | 〃 |

- 앱 코드는 안 건드린다. `pickLang()`이 **저장값 최우선**이라, 랜딩이 `localStorage`에 언어를
  심고 보내면 앱이 그 언어로 뜬다.
- `og:url`은 **랜딩 자신**을 가리킨다. `/`로 두면 FB·디스코드가 og:url을 정본으로 보고 그쪽을
  다시 긁어 **미리보기가 한국어로 덮인다.** 검색엔진용 정본은 `<link rel=canonical>`이 따로 맡는다.
- nginx `try_files $uri $uri.html` 덕에 확장자 없이 `/en`으로 쓴다. `/en/`(슬래시)도
  301로 `/en`에 붙는다 — 이게 없으면 `/en/`이 index.html로 폴백되는데, 자산이 상대경로라
  브라우저가 `/en/app/…`을 찾아 **앱이 통째로 죽는다.**

> `index.html`의 `<base href="/">`를 지우지 말 것. 위 사고를 막는 안전망이다
> (ES 모듈 import는 각 모듈 URL 기준이라 영향 없고, HTML 안의 상대 URL만 잡는다).

### 실제 배포 (life-reroll.com) — Cloudflare Pages

캠프 VM 회수로 **Cloudflare Pages + Pages Functions + D1** 으로 옮겼다.
절차·구조·컷오버는 [docs/CLOUDFLARE-PAGES.md](docs/CLOUDFLARE-PAGES.md) 가 정본이다.

- **배포 = `main` 푸시.** Pages 가 `pnpm run build:core` 를 돌리고 `apps/web` 을 올린다.
  다른 브랜치·PR 은 자동으로 미리보기 URL 을 받는다.
- `/api/` 11종은 [functions/api/](functions/api/) (구 `server/counter.js` 의 이식본),
  저장은 D1. **`/api/track` 은 dwell·roll 을 저장하지 않는다** — 둘이 트래픽의 98%라
  무료 쓰기 한도를 넘기고, 실제 분석은 전부 나머지 2%에서 나왔다.
- 옛 GitHub Pages 금지 사유("정적만 되니 API 가 죽는다")는 **Functions 로 해소**됐다.

VM 시절 nginx 가 지키던 규칙은 전부 파일로 옮겨져 레포에 산다 — 지우면 그때의 사고가 돌아온다:

| 파일 | 구 nginx | 지키는 것 |
|---|---|---|
| [apps/web/_headers](apps/web/_headers) | `.css/.js` no-cache | 낡은 JS 가 캐시에 박히는 것 |
| [apps/web/404.html](apps/web/404.html) | `try_files $uri =404` | 없는 `.js` 에 index.html 이 200 으로 나가 CDN 을 오염시키는 것 |
| [apps/web/_redirects](apps/web/_redirects) | `/en/` → `/en` 301 | `/en/` 폴백으로 자산 경로가 깨져 앱이 죽는 것 |
| [functions/_middleware.js](functions/_middleware.js) | geo 쿠키 | 접속 국가 기반 언어 자동 선택 (없으면 전부 영어) |
| [apps/web/_routes.json](apps/web/_routes.json) | — | 정적 자산이 함수 호출을 태워 무료 한도(10만/일)를 새게 하는 것 |

### 실험판(lab) — 제거됨

VM 전용 인프라(`lab.sh` · `/lab/` · `/lab-api/` · git worktree)였고 VM 회수와 함께 없앴다.
Cloudflare Pages 는 **모든 PR·브랜치에 미리보기 URL 을 자동 발급**하므로 같은 역할을
훨씬 싸게 대신한다 — 격리(별도 오리진 = 별도 localStorage)도 공짜로 따라온다.
마지막 구현은 git 이력(`lab.sh` 삭제 커밋 직전)에 있다.

## 광고 (Google AdSense)

기본은 **꺼짐**이다. 게시자 ID를 넣기 전까지 광고 슬롯은 DOM에서 제거되고 외부 스크립트도
부르지 않아, 리롤 체감이나 모바일 한 화면 레이아웃이 전혀 바뀌지 않는다.

**켜는 법** (애드센스 승인 후):

1. [app/ui/ads.js](app/ui/ads.js)에서 `ADSENSE_CLIENT`(게시자 ID `ca-pub-…`)와 `ADSENSE_SLOT`(광고 단위 슬롯 ID)를 채운다. 둘 중 하나라도 비면 광고는 완전히 꺼진 것으로 본다.
2. 루트의 [ads.txt](ads.txt)에서 `pub-0000000000000000`을 본인 게시자 번호로 바꾼다.
3. 배포(`sudo ./deploy.sh`) — `ads.txt`도 배포 목록에 있다.

**설계**: 광고는 화면 밖(통계·확률표 아래)에만 둔다 — 리롤 결과·버튼과 분리된 위치라
스크롤해야 나온다. 스크립트는 `defer`로 첫 페인트 이후 + 사용자가 광고 근처까지
스크롤했을 때만(`IntersectionObserver`) 로드한다. 애드블록 등으로 못 불러오면 자리도 없앤다.
"광고" 라벨은 애드센스의 콘텐츠·광고 구분 정책 요건이라 뺄 수 없다.

**⚠ 실제 수익까지의 벽** (넣기 전에 알아둘 것):

- **승인 불확실.** 단일 기능 페이지는 "저품질 콘텐츠"로 반려되기 쉽다. 읽을거리(가이드·데이터 설명)를 늘려야 승산이 생긴다.
- **ads.txt.** life-reroll.com 은 루트 도메인이라 레포의 [ads.txt](ads.txt)가 그대로 정본이다 — 서브도메인 시절 필요했던 루트 위임(운영진 협조)은 더 이상 필요 없다.
- **지급 문턱 $100.** 누적 수익이 이를 넘어야 입금된다.

## 데이터 출처와 한계

인구·도시화율·기대수명·1인당 GDP는 UN World Population Prospects 2024,
세계은행 등 공개 통계 기반 **근사치**를 코드에 내장했다 ([index.html](index.html)의 `RAW` 배열).
종교 분포는 국가 대표값이고, 연 소득(USD)은 1인당 GDP에 로그정규 편차를 더한
추정이라 실제 개인 소득이 아니다. 오락용이며 학술적 정확성을 보장하지 않는다.

민족·키·몸무게는 `BODY` 배열에 있다. **"인종"이라는 전 세계 공통 분류는 존재하지 않는다** —
나라마다 스스로를 나누는 기준이 다르다(자기신고·조상·부족·언어). 그래서 각 나라가 실제로 쓰는
구분을 그대로 따랐고(미국은 인구조사 범주, 나이지리아는 종족, 한국은 단일 민족), **나라끼리
비교할 수 있는 값이 아니다.** CIA World Factbook 등 공개 자료 기반 근사치이며 소국일수록 오차가 크다.
키는 NCD-RisC 계열 자료 기반의 국가·성별 평균에 개인 편차(표준편차 남 7cm / 여 6.4cm)를 더했고,
몸무게는 국가 평균 BMI에 개인 편차를 더해 뽑은 뒤 그 사람의 키로 역산한다 — 소득과 같은 방식의 추정이다.

### 탈모를 나라에 연결하지 않는 이유

탈모(안드로겐성 탈모)도 **성별로만 뽑는다** — 50세까지 남성 약 50% · 여성 약 20%.
성별이 압도적인 변수라서이기도 하고, 무엇보다 **키(NCD-RisC)와 달리 198개국을 덮는 조사가
없기 때문이다.** 동아시아 남성이 유럽계보다 낮다는 건 여러 연구에서 반복 확인되지만,
조사마다 연령대와 기준(Norwood 등급 컷오프)이 달라 국가별 숫자로 옮기면 대부분 지어낸 값이 된다.
있는 데이터의 한계를 그대로 인정하고 성별까지만 반영했다.

### IQ를 나라에 연결하지 않는 이유

IQ는 **어느 나라에서 태어나든 같은 분포(평균 100·표준편차 15)에서 뽑는다.** 탈모처럼
국가와 끊어 둔 항목이다. 다른 항목과 달리 일부러 국가와 끊어 둔 것이라 이유를 남긴다.

IQ 점수는 애초에 **어떤 집단에서든 평균이 100이 되도록 규준화한 값**이다. 그래서 "이 나라
평균 IQ"라는 것은 이 척도 안에 정의되지 않는다. 국가별 IQ 순위를 싣는 자료(Lynn·Vanhanen
계열, 이를 재게시하는 온라인 IQ 테스트 사이트들)가 있긴 하지만 쓰지 않는다:

- 상당수 국가 값이 **실측이 아니라 이웃 나라 값으로 채운 추정치**다.
- 실측이 있는 곳도 표본이 수십 명대이거나 특정 도시·연령·학력에 쏠려 있어 국민을 대표하지 못한다.
- 온라인 테스트 기반 순위는 **자기선택 표본**이다. 인터넷이 되고, 그 언어를 읽고, 재미로 IQ
  테스트를 눌러 본 사람만 잡힌다. 나라를 비교하는 게 아니라 인터넷 이용자 구성을 비교하게 된다.
- 점수는 10년에 약 3점씩 올라가서(플린 효과) 측정 연도가 다른 나라끼리는 애초에 비교가 안 된다.

무엇보다 이 앱에는 **민족 칩이 바로 옆에 있다.** 국가별 IQ를 얹는 순간 "이 민족은 IQ 몇"을
서비스가 사실인 양 단언하게 되는데, 그건 위 자료가 실제로 쓰여 온 용도이기도 하다.
나라에 따라 갈리는 "타고나는 조건"을 더 보여주고 싶다면 **교육 연한(UNDP 기대 교육 연수)**
처럼 실제로 측정된 값이 대안으로 적합하다.

개선 이력과 다음 실험 후보는 [IMPROVEMENT_LOG.md](IMPROVEMENT_LOG.md)에 기록한다.
