...
| 기능 | 설명 |
|---|---|
| 환생 뽑기 | 198개국 인구 가중 추첨 + 성별(출생 성비)·도시/농촌(도시화율)·모국어·민족·종교·키·몸무게·IQ·왼손잡이·기대수명·연 소득(USD). 누르면 즉시 결과가 나온다 |
...
전송과 "모두의 환생 횟수" 증가만 건너뛴다 ([js/util.js](js/util.js)의 `isAutomated`).
