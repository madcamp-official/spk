...
app/                (예전 이름은 js/ 였다 — CDN이 옛 파일을 4시간 물고 있어 한 번 옮겼다)
  ...
  i18n/       표시 언어·번역 사전 (아래 "언어 규칙" 참고)
    i18n.js       표시 언어(ko/en/ja/zh)·UI 문구 사전·전환 위젯
...
- **서브도메인 + ads.txt.** IAB 규격상 크롤러는 루트(`madcamp-kaist.org`)의 ads.txt를 먼저 본다. 루트에 `subdomain=life-reroll.madcamp-kaist.org` 위임 한 줄이 있어야 이 서브도메인 ads.txt가 인정된다 — 이 줄은 **루트 도메인 소유자(캠프 운영진)만** 넣을 수 있다.
