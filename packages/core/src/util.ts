...
export function setRNG(f: () => number): void { RNG = f; }
...
/** [값, 가중치] 쌍에서 가중 추첨 */
export function pickWeighted<T extends [string, number]>(pairs: T[]): T {
  let sum = 0;
  for (const p of pairs) sum += p[1];
  let r = rand() * sum;
  for (const p of pairs) { r -= p[1]; if (r <= 0) return p; }
  return pairs[0];
}
...
export function isoCode(flag: string): string {
  return [...flag].map(ch => String.fromCodePoint((ch.codePointAt(0) as number) - 0x1F1E6 + 65)).join("");
}
