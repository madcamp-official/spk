...
function femForm(rule: Culture["fem"], p: NamePart): NamePart {
  ...
  if (rule === "a") { if (/(ov|ev|in)$/.test(l)) out = l + "a"; }
    ...
    else if (l.endsWith("ek")) out = l.slice(0, -2) + "ková";
    else out = l + "ová";
  }
  return { n: out, l: out };
}
...
function tr(latin: string, culture: string, idx: 0 | 1): string {
  ...
  if (/(ov|ev|in)a$/.test(latin)) {
    ...
    if (base) {
      const b = base[idx];
      if (idx === 0 && b.endsWith("프")) return b.slice(0, -1) + "바";
      if (idx === 1 && b.endsWith("フ")) return b.slice(0, -1) + "ワ";
    }
  }
  if (latin.endsWith("ska")) {
    ...
    if (base) {
      const b = base[idx];
      if (idx === 0 && b.endsWith("키")) return b.slice(0, -1) + "카";
      if (idx === 1 && b.endsWith("キ")) return b.slice(0, -1) + "カ";
    }
  }
  return latin;
}
/** 한글/가타카나 음역 전체 이름. ko는 띄어쓰기(중국계 성이름은 붙임), ja는 ・ 구분. */
function transliterated(name: LifeName, lang: "ko" | "ja"): string {
  const cult = POOLS[name.culture]!;
  const idx: 0 | 1 = lang === "ko" ? 0 : 1;
  const sep = lang === "ko" ? " " : "・";
...
export function formatLifeName(name: LifeName, lang: UiLang): string {
  ...
  if (lang === "ko") return transliterated(name, "ko");
  if (lang === "ja") {
    if (cult.native === "zh") return nativeForm(name) ?? latinForm(name);
...
export function altLifeName(name: LifeName, lang: UiLang): string | null {
  ...
  if (main !== latin) return latin;
  const native = nativeForm(name);
  return native && native !== main ? native : null;
}
/** 검증용: 사전에 없는(로마자로 폴백하는) 조각 목록. 비어 있어야 한다. */
export function missingTransliterations(): string[] {
  const miss = new Set<string>();
  const probe = (latin: string, culture: string) => {
    for (const idx of [0, 1] as const) {
      const out = tr(latin, culture, idx);
      if (out === latin) miss.add(latin);
    }
  };
  for (const [key, cult] of Object.entries(POOLS)) {
    for (const e of [...cult.m, ...cult.f]) probe(part(e).l, key);
    const style = cult.style ?? "family";
    if (style === "patronym_is") {
      for (const e of cult.s!) { probe(part(e).l + "son", key); probe(part(e).l + "dóttir", key); }
    } else if (cult.s) {
      for (const e of cult.s) {
        const p = part(e);
        probe(p.l, key);
        if (style === "family" && cult.fem) probe(femForm(cult.fem, p).l, key);
      }
    }
  }
  return [...miss];
