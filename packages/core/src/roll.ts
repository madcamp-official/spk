...
import type { Cause, CauseInput, Country, Life, Trait, WeightedPair } from "./types.js";
...
const CAUSES: { key: string; emoji: string; wt: (a: number, m: boolean) => number }[] = [
...
export function rollCause(l: CauseInput): Cause {
  ...
  for (let i = 0; i < CAUSES.length; i++) {
    r -= w[i]!;
    if (r <= 0) return { key: CAUSES[i]!.key, emoji: CAUSES[i]!.emoji };
  }
...
export function rollLife(fCi?: number | null, fMale?: boolean | null): Life {
  ...
  const l: Life = { ci, c, male, urban, rel, eth, balding, lifeExp, income, top, iq, ...body,
...
export const TRAIT_KEYS = ["longevity", "wealth", "rare_land", "genius"] as const;
