...
export function incomeTopPct(v) {
    let a = 0;
    for (const c of DATA)
        a += (c.pop / TOTAL) * phi((Math.log(v) - Math.log(c.gdp)) / SAMPLING.incomeLogSigma);
    return clamp((1 - a) * 100, 0.01, 99.9);
}
export function rarityColor(pop) {
...
export function rollBody(c, male) {
    const height = clamp(Math.round((male ? c.hm : c.hf) + gauss() * (male ? SAMPLING.heightSigmaCm.male : SAMPLING.heightSigmaCm.female)), SAMPLING.heightClamp[0], SAMPLING.heightClamp[1]);
...
export function rollIQ() {
    return clamp(Math.round(SAMPLING.iq.mean + gauss() * SAMPLING.iq.sigma), SAMPLING.iq.clamp[0], SAMPLING.iq.clamp[1]);
}
export function iqTopPct(iq) {
...
/* 사망 원인 — 죽는 나이(기대수명)에 따라 분포가 크게 달라진다. 젊어 죽을수록 사고·감염병,
   ...
   국가별 사인 통계는 198개국을 고르게 덮지 못해(탈모와 같은 사정) 나라로 가르지 않고
   죽는 나이 하나로만 기운다.
...
export function rollCause(l) {
    const seed = strHash([isoCode(l.c.flag), l.lifeExp, Math.round(l.income), l.iq, l.height,
        Math.round(l.weight * 10)].join("-"));
    const rng = mulberry32(seed);
    const a = l.lifeExp, m = !!l.male;
    let sum = 0;
    const w = CAUSES.map(cs => { const x = Math.max(0, cs.wt(a, m)); sum += x; return x; });
    let r = rng() * sum;
    for (let i = 0; i < CAUSES.length; i++) {
        r -= w[i];
        if (r <= 0)
            return { key: CAUSES[i].key, emoji: CAUSES[i].emoji };
    }
    return { key: CAUSES[0].key, emoji: CAUSES[0].emoji };
}
...
export function rollLife(fCi, fMale) {
    ...
    const lifeExp = clamp(Math.round(c.life + gauss() * SAMPLING.lifespanSigmaYears), SAMPLING.lifespanClamp[0], SAMPLING.lifespanClamp[1]);
...
export function rollSiblings(_c) {
...
export const TRAIT_KEYS = ["longevity", "wealth", "rare_land", "genius"];
